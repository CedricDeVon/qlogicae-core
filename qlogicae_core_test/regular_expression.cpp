...
namespace QLogicaeCoreTest
{
    ...
    {
        bool result = QLogicaeCore::RegularExpression::instance().add_pattern("invalid", "[");
        ASSERT_FALSE(result);
    }

    TEST(RegularExpressionTest, Should_ReturnTrue_When_AddingValidPattern)
    {
        bool result = QLogicaeCore::RegularExpression::instance().add_pattern("valid", "abc");
        ASSERT_TRUE(result);
    }

    TEST(RegularExpressionTest, Should_ReturnEmptyString_When_GettingNonexistentPattern)
    {
        std::string pattern = QLogicaeCore::RegularExpression::instance().get_patterrn("none");
        ASSERT_TRUE(pattern.empty());
    }

    TEST(RegularExpressionTest, Should_ReturnPattern_When_GettingExistingPattern)
    {
        QLogicaeCore::RegularExpression::instance().add_pattern("named", "abc");
        std::string pattern = QLogicaeCore::RegularExpression::instance().get_patterrn("named");
        ASSERT_EQ(pattern, "abc");
    }

    TEST(RegularExpressionTest, Should_ReturnTrue_When_PatternExists)
    {
        QLogicaeCore::RegularExpression::instance().add_pattern("exists", "abc");
        ASSERT_TRUE(QLogicaeCore::RegularExpression::instance().has_pattern("exists"));
    }

    TEST(RegularExpressionTest, Should_ReturnFalse_When_PatternDoesNotExist)
    {
        ASSERT_FALSE(QLogicaeCore::RegularExpression::instance().has_pattern("missing"));
    }

    TEST(RegularExpressionTest, Should_ReturnTrue_When_NamedPatternMatchesSubject)
    {
        QLogicaeCore::RegularExpression::instance().add_pattern("match", "abc");
        ASSERT_TRUE(QLogicaeCore::RegularExpression::instance().match_named("abc", "match"));
    }

    TEST(RegularExpressionTest, Should_ReturnFalse_When_NamedPatternNotFound)
    {
        ASSERT_FALSE(QLogicaeCore::RegularExpression::instance().match_named("abc", "missing"));
    }

    TEST(RegularExpressionTest, Should_ReturnTrue_When_DirectMatch)
    {
        ASSERT_TRUE(QLogicaeCore::RegularExpression::instance().match_direct("abc", "abc"));
    }

    TEST(RegularExpressionTest, Should_ReturnFalse_When_DirectMatchFails)
    {
        ASSERT_FALSE(QLogicaeCore::RegularExpression::instance().match_direct("xyz", "abc"));
    }

    TEST(RegularExpressionTest, Should_ReturnFalse_When_DirectMatchPatternInvalid)
    {
        ASSERT_FALSE(QLogicaeCore::RegularExpression::instance().match_direct("abc", "["));
    }

    TEST(RegularExpressionTest, Should_CompleteUnderTimeLimit_When_AsyncMatchNamed)
    {
        QLogicaeCore::RegularExpression::instance().add_pattern("async", "abc");
        auto future = QLogicaeCore::RegularExpression::instance().match_named_async("abc", "async");
    ...
    {
        auto future = QLogicaeCore::RegularExpression::instance().match_direct_async("abc", "abc");
        ASSERT_TRUE(future.wait_for(std::chrono::seconds(2)) == std::future_status::ready);
        ASSERT_TRUE(future.get());
    }
    ...
    {
        QLogicaeCore::RegularExpression::instance().add_pattern("toClear", "abc");
        QLogicaeCore::RegularExpression::instance().clear_all_patterns();
        ASSERT_FALSE(QLogicaeCore::RegularExpression::instance().has_pattern("toClear"));
    }
    ...
    TEST_P(RegularExpressionTest, Should_MatchCorrectly_When_Parameterized)
    {
        ...
        ASSERT_EQ(QLogicaeCore::RegularExpression::instance().match_named(subject, name), expected);
    }
    ...
    TEST(RegularExpressionTest, Should_HandleStress_When_RepeatingAddAndMatch)
    {
        for (int index = 0; index < 10000; ++index)
        {
            ...
            QLogicaeCore::RegularExpression::instance().add_pattern(name, "abc");
            ASSERT_TRUE(QLogicaeCore::RegularExpression::instance().match_named("abc", name));
        }
    }
    ...
    TEST(RegularExpressionTest, Should_HandleConcurrentAccess_When_MultipleThreads)
    {
        ...
        QLogicaeCore::RegularExpression::instance().add_pattern("threaded", "abc");

        auto task = []()
            {
                for (int index = 0; index < 1000; ++index)
                {
                    if (!QLogicaeCore::RegularExpression::instance().match_named("abc", "threaded"))
                    {
                        return false;
                    }
                }
    ...
    TEST(RegularExpressionTest, Should_HandleConcurrentAddClearMatch_When_MultipleThreads)
    {
        ...
        std::thread matcher([&]() {
            for (int i = 0; i < 1000; ++i)
            {
                QLogicaeCore::RegularExpression::instance().match_named("abc", "pattern1");
            }
            });
    ...
    TEST(RegularExpressionTest, Should_NotLeakMemory_When_ReplacingPatternsRepeatedly)
    {
        ...
        {
            ASSERT_TRUE(QLogicaeCore::RegularExpression::instance().add_pattern("repeat", "abc"));
        }
        ASSERT_TRUE(QLogicaeCore::RegularExpression::instance().match_named("abc", "repeat"));
    }
    ...
    TEST(RegularExpressionTest, Should_HandleLongAndUTF8Strings_When_Matching)
    {
        ...
        ASSERT_TRUE(QLogicaeCore::RegularExpression::instance().match_direct(long_subject, long_pattern));
    }
}
