...
namespace QLogicaeCore
{
    ...
    const jwt::algorithm::es256k& JsonWebTokenCryptographer::_get_es256k_signer(
    ...
    const jwt::algorithm::es256k& JsonWebTokenCryptographer::_get_es256k_verifier(
        ...
        JsonWebTokenReverseInput options) const
    {
        ...
        if (options.token.empty()) {
            result.message = "Token input is empty";
            return result;
        }

        if (options.issuer.empty()) {
            result.message = "Expected issuer is empty";
            return result;
        }

        if (options.public_key.empty()) {
            result.message = "Public key is empty";
            return result;
        }
