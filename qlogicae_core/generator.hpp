...
namespace QLogicaeCore
{
    ...
    public:
        ...
        std::vector<std::string> random_string_vector(const size_t&, const size_t& = Constants::RANDOM_SIZE_T_BITS);
        std::string random_hex(const size_t&, const std::string_view & = Constants::FULL_VISIBLE_ASCII_CHARACTERSET);
        std::string random_string(const size_t&, const std::string_view& = Constants::FULL_VISIBLE_ASCII_CHARACTERSET);
        std::string random_base64(const size_t&, const std::string_view & = Constants::FULL_VISIBLE_ASCII_CHARACTERSET);
